  ...
  return (
    <main className="bg-[#0b0e14] text-white min-h-screen pb-24 font-sans selection:bg-purple-500/30">
      ...
      <div className="px-6 mb-8">
        <button onClick={() => window.location.href = '/login'} className="w-full bg-gradient-to-r from-emerald-500 to-green-600 py-4 rounded-2xl font-black text-base shadow-xl shadow-green-900/20 active:scale-[0.98] transition-transform">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#131722]/90 backdrop-blur-xl border-t border-[#1f2937] p-4 flex gap-4 items-center z-50">
        <div className="flex-1"><p className="text-[8px] text-gray-400 font-bold uppercase">Start Earning Today</p><p className="text-sm font-black text-emerald-400">No Complex Approval</p></div>
        <button onClick={() => window.location.href = '/login'} className="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-3 rounded-xl text-xs font-black uppercase shadow-lg shadow-purple-900/30">SIGN UP</button>
      </div>
    </main>
  );
          }
