      ...
      {showModal && currentStep !== 4 && (
        <div className="fixed inset-0 bg-gray-500/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-5 w-full max-w-[340px] text-center shadow-2xl relative">
            <button onClick={() => setShowModal(false)} className="absolute -top-3 -right-1 bg-white border border-gray-300 rounded-full w-8 h-8 flex items-center justify-center text-gray-600 shadow-md font-bold text-lg">×</button>
          ...
          <>
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-md w-full">
              ...
              <div className="space-y-3">
                <button 
                  ...
                  className={`w-full flex items-center justify-center p-4 rounded-xl font-bold transition-all gap-2 ${
                    timer > 0 ? 'bg-gray-100 border border-gray-300 text-gray-400 cursor-not-allowed' : 'bg-[#0275d8] border border-[#0275d8] text-white hover:bg-blue-700 shadow-md'
                  }`}
                >
                  {timer > 0 ? `WAIT ${timer}s` : isFetching ? <><span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>VERIFYING...</> : <><span className="text-lg">🔗</span> GET YOUR DESTINATION LINK</>}
                </button>
                <button onClick={() => window.location.href = "/"} className="w-full flex items-center justify-center gap-2 p-4 rounded-xl font-bold bg-white border-2 border-purple-500 text-purple-600 hover:bg-purple-50 transition-all shadow-sm">
              ...
              <div className="flex justify-between mt-8 text-center divide-x divide-gray-200">
                <div className="flex-1 px-1"><span className="text-blue-500 text-xl block mb-1">🛡️</span><p className="text-[11px] text-gray-800 font-bold">100% Secure</p></div>
                <div className="flex-1 px-1"><span className="text-blue-500 text-xl block mb-1">⚡</span><p className="text-[11px] text-gray-800 font-bold">Super Fast</p></div>
                <div className="flex-1 px-1"><span className="text-emerald-500 text-xl block mb-1">💵</span><p className="text-[11px] text-gray-800 font-bold">Earn Money</p></div>
              </div>
            </div>
