...
export default function StepPage() {
  ...
  const handleContinue = async () => {
    if (currentStep < 4) {
      ...
      // STEP 4: Firebase se Original URL fetch karna
      setIsFetching(true);
      try {
        // Yahan "links" aapke Firestore collection ka naam hai. 
        // Agar aapne database me collection ka naam kuch aur rakha hai (jaise "urls"), to ise change kar lena.
        const linksRef = collection(db, "links");
        
        // Query: Jahan "alias" field ki value humare 'code' se match karti ho
        ...
        if (!querySnapshot.empty) {
          ...
          // Yahan "url" aapke original link wale field ka naam hai. 
          // Ise apne database field ke hisaab se update karein (e.g., data.originalUrl, data.longLink)
          if (data.url) {
            window.location.href = data.url; 
          } else {
            alert("URL field not found in database!");
            setIsFetching(false);
          }
        } else {
    ...
    <div className="min-h-screen bg-[#f1f1f1] text-gray-800 font-sans relative">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-500/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-5 w-full max-w-[340px] text-center shadow-2xl relative">
            <button 
              onClick={() => { console.log("Fake close clicked"); }} 
      ...
      <main className="max-w-3xl mx-auto mt-6 px-4 pb-12 flex flex-col items-center space-y-6">
        ...
        <div className="w-full text-center w-full md:w-1/2 mx-auto">
          ...
          >
            {timer > 0 ? `Please Wait... ${timer}s` : (isFetching ? "FETCHING URL..." : (currentStep === 4 ? "GET DESTINATION LINK" : "CONTINUE NEXT STEP"))}
          </button>
        </div>
