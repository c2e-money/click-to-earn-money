...
export default function Dashboard() {
  ...
  const handleGenerate = async () => {
    ...
    try {
        await setDoc(doc(db, "urls", shortCode), { code: shortCode, originalUrl: url.startsWith('http') ? url : `https://${url}`, userId: user.uid, clicks: 0 });
    ...
    <div className="flex flex-col h-screen bg-[#0b0e14] text-white">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#131722] p-4 rounded-2xl border border-[#1f2937]"><p className="text-[9px] font-bold text-gray-500 uppercase">Total Clicks</p><p className="text-lg font-black">{stats.clicks}</p></div>
          ...
          <div className="bg-[#131722] p-4 rounded-2xl border border-[#1f2937]"><p className="text-[9px] font-bold text-gray-500 uppercase">Current CPM</p><p className="text-lg font-black">${stats.cpm.toFixed(2)}</p></div>
        ...
        <div className="bg-[#131722] p-4 rounded-2xl border border-[#1f2937]">
          ...
          {generatedLink && (
            <div className="mt-4 flex items-center gap-2 bg-[#0b0e14] p-3 rounded-xl border border-purple-500">
                ...
                <button onClick={() => navigator.clipboard.writeText(generatedLink)} className="bg-purple-600 px-3 py-1 rounded-lg text-[10px] font-black">COPY</button>
            </div>
          )}
        </div>
