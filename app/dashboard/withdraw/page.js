...
export default function Withdraw() {
  ...
  const [history] = useState([]); // Khali array rakhne se history tabhi dikhegi jab data hoga
          ...
          {history.length === 0 ? (
            ...
            history.map((item) => (
              <div key={item.id} className="bg-[#131722] p-4 rounded-2xl border border-[#1f2937] flex justify-between items-center">
                ...
                <span className="text-[9px] font-black bg-yellow-900/30 text-yellow-500 px-2 py-1 rounded uppercase">{item.status}</span>
              </div>
            ))
          )}
        </div>
