...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.isBanned) {
    return res.status(403).json({ error: "Your account is banned. Contact dipenshorts@gmail.com." });
  }
...
app.post("/api/admin/config", requireAdmin, (req, res) => {
  const { globalCpm } = req.body;
  if (globalCpm === undefined || isNaN(parseFloat(globalCpm))) {
    return res.status(400).json({ error: "Invalid global CPM rate" });
  }
  const config = updateSystemConfig(parseFloat(globalCpm));
  res.json(config);
});
