...
import { Mail, Lock, User as UserIcon, LogIn, ChevronRight, AlertCircle, Coins, BarChart3, ShieldCheck, Zap } from 'lucide-react';
...
export default function RegisterPage({ onRegisterSuccess, onNavigate }: RegisterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.user) {
        ...
        setError(data.error || 'Failed to register. Please check credentials or try another username.');
      }
    } catch (err) {
      setError('Network connection error. Try again later.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-slate-950 text-slate-100 min-h-[calc(100vh-73px)] flex items-center justify-center p-4 relative overflow-hidden" id="register-page-root">
      ...
      <div className="relative max-w-5xl w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-center z-10" id="register-container-grid">
        
        {/* Left Side: Premium Cyberpunk Info-Section */}
        <div className="lg:col-span-7 flex flex-col justify-center space-y-6 p-6 md:p-8 rounded-2xl bg-slate-900/40 border border-cyan-500/15 backdrop-blur-md shadow-[0_0_50px_rgba(6,182,212,0.05)]" id="register-info-section">
          <div>
          ...
          <div className="space-y-4" id="register-platform-trust-badges">
        ...
        <div className="lg:col-span-5 p-6 md:p-8 rounded-2xl bg-slate-900 border border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.6)]" id="register-form-box">
          <div className="mb-6">
            <h3 className="text-2xl font-black text-white">Register An Account</h3>
            <p className="text-xs text-slate-400 mt-1">Start shortening URLs and monetizing your traffic in seconds.</p>
          </div>

          {error && (
            <div className="mb-4 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs flex items-center space-x-2 animate-shake" id="register-error-alert">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" id="register-form">
            {/* Username */}
            <div>
              <label className="text-xs text-slate-300 font-bold uppercase tracking-wider block mb-1.5">Username</label>
              <div className="relative">
                <UserIcon className="absolute left-3.5 top-3 w-4 h-4 text-slate-500" />
                <input
                  type="text"
                  required
                  placeholder="e.g. dipen_shorts"
                  value={username}
