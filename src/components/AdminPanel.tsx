...
import { 
  ...
  Clock, CheckCircle2, XCircle, FileText, ArrowDownToLine
} from "lucide-react";
import { 
  listenToAdminStats, 
  listenToAllUsers, 
...
export default function AdminPanel({ userUid }: AdminPanelProps) {
  const [activeTab, setActiveTab] = useState<"users" | "links" | "withdrawals" | "analytics">("users");
  ...
  useEffect(() => {
    ...
    // Real-time global CPM rate config listener
    const unsubCpm = listenToGlobalCpm(userUid, (rate) => {
      setGlobalCpmRate(rate.toFixed(2));
    });
  ...
  const handleUpdateCpm = async () => {
    ...
    setSavingCpm(true);
    try {
      await updateGlobalCpm(userUid, parsed);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5 max-w-md mx-auto pb-12 px-4" id="admin-panel-viewport">
      ...
      {/* ADMIN TABS SWITCHER */}
      <div className="grid grid-cols-4 gap-1 bg-[#131a2e] p-1 rounded-xl border border-slate-800">
        <button
