...
import { listenToSystemConfig } from "../lib/firebaseService";

interface SafeHTMLAdRendererProps {
  htmlCode: string;
  className?: string;
  id?: string;
}

function SafeHTMLAdRenderer({ htmlCode, className, id }: SafeHTMLAdRendererProps) {
  ...
  useEffect(() => {
    ...
    // Append child nodes and re-create any script tag so it executes
...
export default function RedirectLoop({ shortCode }: RedirectLoopProps) {
  ...
  // Subscribe to real-time global system configuration
  useEffect(() => {
    const unsubscribe = listenToSystemConfig("guest", (config) => {
      setSystemConfig(config);
      setGlobalCpm(config.globalCpm || 5.00);
    });
    return () => unsubscribe();
  }, []);

  // Load Social Bar and Popunder scripts dynamically in the background
  useEffect(() => {
    const scriptsToClean: HTMLScriptElement[] = [];

    // 1. POPUNDER SCRIPT (if configured and step !== 5)
    if (step !== 5 && systemConfig.popunderCode) {
      // Check if popunderCode is an HTML script snippet or just a raw URL/script path
      const isHtmlScript = systemConfig.popunderCode.includes("<script");
      const isScriptPath = systemConfig.popunderCode.endsWith(".js") || systemConfig.popunderCode.includes("rightyrely.com");
      
      if (isHtmlScript || isScriptPath) {
        const tempDiv = document.createElement("div");
        tempDiv.innerHTML = isHtmlScript 
          ? systemConfig.popunderCode 
          : `<script type="text/javascript" src="${systemConfig.popunderCode}"></script>`;
        
        const scriptElements = tempDiv.getElementsByTagName("script");
        Array.from(scriptElements).forEach((s) => {
          const newScript = document.createElement("script");
          newScript.id = "ad-popunder-node";
    ...
    // 2. SOCIAL BAR SCRIPT (if configured)
    if (systemConfig.socialBarCode) {
      const isHtmlScript = systemConfig.socialBarCode.includes("<script");
      const tempDiv = document.createElement("div");
      tempDiv.innerHTML = isHtmlScript
        ? systemConfig.socialBarCode
        : `<script type="text/javascript" src="${systemConfig.socialBarCode}"></script>`;

      const scriptElements = tempDiv.getElementsByTagName("script");
      Array.from(scriptElements).forEach((s) => {
        const newScript = document.createElement("script");
        newScript.id = "ad-social-node";
        Array.from(s.attributes).forEach(attr => newScript.setAttribute(attr.name, attr.value));
        newScript.textContent = s.textContent;
        document.body.appendChild(newScript);
        scriptsToClean.push(newScript);
    ...
    return () => {
      ...
      document.getElementById("ad-social-node")?.remove();
      const extraScripts = document.querySelectorAll(`script[src*="rightyrely.com"], script[src*="highperformanceformat.com"]`);
      extraScripts.forEach(el => el.remove());
    };
  }, [step, systemConfig.popunderCode, systemConfig.socialBarCode]);
  ...
  const handleAdClick = (adUrl?: string) => {
    ...
    setTimeLeft(0);
    try {
      const targetUrl = adUrl && adUrl !== "https://example.com" 
        ? adUrl 
        : systemConfig.popunderCode || "https://www.profitablecpmrate.com/watchnew?key=f313df2db66a1ec18991469eef68f071";
  ...
  return (
    <div className="min-h-screen bg-[#060911] text-slate-100 flex flex-col relative pb-32">
      ...
      <div className="flex-1 max-w-md mx-auto w-full px-4 py-6 space-y-5">
        ...
        {systemConfig.nativeAdCode ? (
          <SafeHTMLAdRenderer 
            htmlCode={systemConfig.nativeAdCode}
            className="w-full flex justify-center py-2 bg-[#0a0915] border border-indigo-500/15 rounded-2xl p-4 shadow-inner"
            id="container-32f9b2e2cd7f03ce9395d5574f524b41"
          />
        ) : null}
        ...
        {/* Real Dynamic Sidebar Banner Ad Slot */}
        {systemConfig.bannerAdCode ? (
          <SafeHTMLAdRenderer 
            htmlCode={systemConfig.bannerAdCode}
            className="w-full flex justify-center py-2 bg-[#0a0915] border border-indigo-500/15 rounded-2xl p-4 shadow-inner"
          />
        ) : null}
